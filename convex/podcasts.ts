...
export const createPodcast = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("podcast", {
...
// this query will get all the podcasts based on the voiceType of the podcast, which is displayed in the Similar Podcasts section.
export const getPodcastByVoiceType = query({
  args: {
    podcastId: v.id("podcast"),
  },
  handler: async (ctx, args) => {
...
export const getAllPodcasts = query({
  handler: async (ctx) => {
    return await ctx.db.query("podcast").order("desc").collect();
  },
});
